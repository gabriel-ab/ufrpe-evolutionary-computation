...
def list_field() -> list[int]:
    def factory() -> list[int]:
        return [random.randint(MIN_NEURONS, MAX_NEURONS) for _ in range(random.randint(MIN_LENGTH, MAX_LENGTH))]
    return dc.field(default_factory=factory)
...
class Individual:
    ...
    def mate_onepoint(self, other):
        ...
        child1 = type(self)(**{attr: getattr(self if i < point else other, attr) for i, attr in enumerate(field_names)})
        child2 = type(self)(**{attr: getattr(other if i < point else self, attr) for i, attr in enumerate(field_names)})
